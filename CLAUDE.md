...
- **Stack:** Django 6 (Python 3.13+), allauth + MFA, Postgres 16, Redis (cache + Celery broker + idempotency lock), gunicorn (uvicorn worker, ASGI), nginx, supervisord, Celery, Vite frontend, django-prometheus.
- **Topology in production:** single container — nginx → unix socket `/tmp/apis.sock` → gunicorn (ASGI). Static files served by nginx.
...
- **Proxy / client-IP chain:** nginx terminates HTTP on port 80, proxies to gunicorn over a unix socket. Because the upstream is a unix socket, gunicorn sees no real `REMOTE_ADDR`. The real client IP must be carried through `X-Forwarded-For` and surfaced into `request.META['REMOTE_ADDR']` by trusted middleware. allauth's `ALLAUTH_TRUSTED_CLIENT_IP_HEADER` is the supported integration point and **must be set** to the header that the trusted edge (Azure Front Door → nginx) populates. See ASVS V14, OWASP "HTTP Headers" and "Logging" cheat sheets.
