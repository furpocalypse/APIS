...
- **Settings layout (Decision #11):** ONE canonical, tracked, env-driven file `fm_eventmanager/settings.py` owns all logic; there is **no `settings_base.py` / `settings_test.py` split** (both deleted). Posture is selected purely by the loaded `.env.${APIS_ENV}` (tracked `.env.test`/`.env.ci`/`.env.dev`; gitignored `.env.production` + tracked `.env.production.example`); `APIS_ENV` is the selector and the `_IS_PROD` guards hard-fail a production process running test/DEBUG posture. The Docker image bakes `settings.py` directly; the dev compose bind-mounts it for hot-reload. `.env.*` files must be bare `KEY=value` (4 parsers: direnv / `set -a;.` / `$GITHUB_ENV` / compose `env_file`).
