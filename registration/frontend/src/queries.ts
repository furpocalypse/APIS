...
export const api = ky.extend({
  ...
  hooks: {
    beforeRequest: [
      (request) => {
        if (MUTATING_METHODS.has(request.request.method.toLowerCase())) {
          if (!request.request.headers.has("idempotency-key")) {
          ...
          if (CSRF_TOKEN) {
            request.request.headers.set("x-csrftoken", CSRF_TOKEN);
          }
        }
      },
    ],
