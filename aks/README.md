...
- **`APIS_ENV` is not currently set by these overlays — the production boot guards are inert here.** Every `_IS_PROD` fail-closed check in `fm_eventmanager/settings.py` (E2E off, DEBUG off, MQTT secret strength, TRUSTED_PROXY_CIDRS placeholder, and the PayPal config completeness assert that prevents the silent 403-all-webhooks misconfiguration) is keyed on `APIS_ENV=production`, which only the docker-compose path sets via `.env.production`. Before relying on those guards in AKS, add `APIS_ENV=production` to the prod overlay's config — and first replace the placeholder `TRUSTED_PROXY_CIDRS` and provision `MQTT_JWT_SECRET`, or the newly armed guards will (correctly) refuse to boot. Note the silent-failure case the PayPal assert exists to catch is an empty-valued or partially-populated Key Vault secret: a wholly absent vault object already fails the CSI mount and the pod won't start.
